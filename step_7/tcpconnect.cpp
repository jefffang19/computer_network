...
Packet::Packet(){
	...
	this->header.option[0] = 0;
	this->header.option[1] = 0;
...
Packet::Packet(packetType type, Tcpconnect tcp, const char* indata, int indataSize){
	...
	switch(type){
		...
		case packet_syn:
			...
			//add sack option here
			this->header.option[0] = 4;
			this->header.option[1] = 2;
			break;
		case packet_synack:
			this->header.ACK = true;
			this->header.SYN = true;
...
void Tcpconnect::mySend(Packet packet, bool safemode){
	...
	if(pt[packet.packet_type()]=="DATA")
		++isloss; //while lost at the 10,12,14th data pack (10240,12288,14336byte)
	//if no loss
	if(isloss!=10 && isloss!=12 && isloss!=14 || safemode) sendto(hostfd, &packet, sizeof(Packet), 0, (struct sockaddr *) &destSocket, sizeof(destSocket) );
	//if loss
	else{
...
bool Tcpconnect::updateNum(const Packet recv_packet){
	if(this->seqNum == recv_packet.header.ackNum - 1){
		...
		bestseqNum = recv_packet.header.ackNum;
		bestseqNum = recv_packet.header.seqNum + 1;
		return false;
	}
}
